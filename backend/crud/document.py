...
def create_document(db: Session, doc_id: str, filename: str) -> DocumentRecord:
    """初始化记录"""
    db_doc = DocumentRecord(id=doc_id, filename=filename, status=DocumentStatus.PENDING.value)
    db.add(db_doc)
    db.commit()
    db.refresh(db_doc)
    return db_doc

def update_document_status(db: Session, doc_id: str, status: str) -> Optional[DocumentRecord]:
...
def update_document_success(db: Session, doc_id: str, predicted_class: str, confidence: float, ocr_results: List[Dict[str, Any]] = None) -> Optional[DocumentRecord]:
    """处理完毕，写入核心分类及特征结果"""
    db_doc = db.query(DocumentRecord).filter(DocumentRecord.id == doc_id).first()
    if db_doc:
...
def update_document_failed(db: Session, doc_id: str, error_msg: str) -> Optional[DocumentRecord]:
    """处理失败的异常挂起处理"""
    db_doc = db.query(DocumentRecord).filter(DocumentRecord.id == doc_id).first()
    if db_doc:
