...
def _handle_file_upload(file: UploadFile) -> str:
    ...
    try:
        ...
        if ext == ".pdf" and file_size_mb > settings.MAX_PDF_SIZE_MB:
            raise HTTPException(
                status_code=400,
                detail=f"PDF 文件大小 ({file_size_mb:.2f}MB) 超出限制 ({settings.MAX_PDF_SIZE_MB}MB)",
            )
        elif ext != ".pdf" and file_size_mb > settings.MAX_IMAGE_SIZE_MB:
            raise HTTPException(
                status_code=400,
                detail=f"图片文件大小 ({file_size_mb:.2f}MB) 超出限制 ({settings.MAX_IMAGE_SIZE_MB}MB)",
            )
...
@router.post(
    "/process",
    response_model=BaseResponse[OCRResponse],
    summary="解析文档图像基础接口",
    description="支持PDF/PNG/JPG/BMP/TIF等格式。单文件≤200MB(PDF)或10MB(图片)。",
)
